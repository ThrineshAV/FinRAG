...
Usage in endpoints::
    ...
    from src.auth.dependencies import require_api_key, require_admin
...
async def require_upload(request: Request) -> APIKeyRecord | None:
    ...
    if record is not None and not record.has_permission("upload"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Upload permission required.  Your key has role "
            f"'{record.role.value}'; an 'admin' key is needed for uploads.",
        )
    return record
